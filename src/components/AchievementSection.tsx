...
import { ChevronLeft, ChevronRight, Calendar, MapPin, User } from "lucide-react";
...
export default function AchievementSection() {
    ...
    if (activeTab === "perlombaan") {
      ...
      return (
        <div className="relative">
          <div className="relative overflow-hidden rounded-2xl group-hover:scale-105 transition-transform duration-500">
            ...
            <div className={`hidden w-full h-48 lg:h-56 bg-gradient-to-br ${colors.iconBg} items-center justify-center rounded-2xl`}>
      ...
      return (
        <div className="relative overflow-hidden rounded-2xl group-hover:scale-105 transition-transform duration-500">
          ...
          />
          {/* Fallback gradient background */}
          <div className={`hidden w-full h-48 bg-gradient-to-br ${colors.iconBg} items-center justify-center rounded-2xl`}>
            <div className={`${colors.iconColor} text-6xl`}>{item.emoji}</div>
          </div>
          {/* Overlay gradient */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>
  ...
  const renderCertificateCard = (item, index) => (
    <div
      ...
      className={`group relative bg-gradient-to-br ${colors.bg} backdrop-blur-sm border ${colors.border} rounded-2xl p-6 transition-all hover:shadow-[0_0_15px_#00ffff] duration-700 ${colors.shadow} ${
    ...
    >
      {/* Glow effect */}
      <div className={`absolute inset-0 bg-gradient-to-r ${colors.glow} to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
  ...
  return (
    <section className="relative min-h-screen w-full px-5 md:px-24 bg-[#0B0D14] py-20 text-white font-mono">
        ...
        <div className="relative bg-black/30 backdrop-blur-sm border border-cyan-400/20 rounded-2xl">
          ...
          <div className="relative flex">
            ...
            >
              ...
              {activeTab === "perlombaan" && (
                <div className={`absolute inset-0 bg-gradient-to-r from-cyan-400/10 to-transparent rounded-xl ${isClient ? 'animate-pulse' : ''}`} />
              )}
            </button>
        ...
        <div className="rounded-2xl p-8">
          {activeTab === "perlombaan" ? (
            ...
            <div className="space-y-6 max-w-4xl mx-auto">
              {displayedData.map((item, index) => renderCompetitionCard(item, index))}
                ...
                <div className="flex justify-center mt-12">
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-center text-cyan-300 font-medium">
                      ...
                      <div className={`transform transition-transform duration-300 ${showAllCompetitions ? "rotate-180" : ""}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {displayedData.map((item, index) => renderCertificateCard(item, index))}
            </div>
          )}
        </div>
      </div>
